...
// Если нет юзера и у него нет корзины - мы создаем корзину и кладем в нее первый товар - выходим
// Если юзер есть и у него есть корзина и у него есть товар этот, то мы увеличиваем счетчик этого товара, если товара нет то добавляем товар и завершаем работу функции
...
func (s *Storage) saveCartsToFile() error {
	...
	// if err = encoder.Encode(s.carts); err != nil {
	// 	return err
	// }

	b, err := json.MarshalIndent(s.carts, "", "\t")
	if err != nil {
		return nil
	}
